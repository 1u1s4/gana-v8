...
const createPrismaClient = async (targetDatabaseUrl: string) =>
  connectPrismaClientWithRetry(new PrismaClient({ datasourceUrl: targetDatabaseUrl }));
  ...
  try {
    await unitOfWork.fixtures.save(
      createFixture({
        ...
        scheduledAt: `2099-01-${suffix.padStart(2, "0")}T18:00:00.000Z`,
        status: "scheduled",
        metadata: {
...
const cleanupRuntimeArtifacts = async (
  ...
  try {
    ...
    if (predictionIds.length || parlayIds.length || taskIds.length || fixtureIds.length) {
    ...
    if (sandboxNamespaceIds.length) {
      await prisma.sandboxNamespace.deleteMany({ where: { id: { in: sandboxNamespaceIds } } });
    }
    ...
    if (cycleIds.length) {
      await prisma.automationCycle.deleteMany({ where: { id: { in: cycleIds } } });
    }
  } finally {
    await prisma.$disconnect();
  }
...
const snapshotSchedulerCursorState = async (
  targetDatabaseUrl: string,
): Promise<ReadonlyMap<string, { id: string; specId: string; lastTriggeredAt: Date | null; metadata: unknown } | null>> => {
  const cursorIds = buildExampleCronSpecs().map((spec) => `scheduler-cursor:${spec.id}`);
  ...
  try {
    ...
    return new Map(
      cursorIds.map((cursorId) => [cursorId, byId.get(cursorId) ?? null] as const),
    );
  } finally {
    await prisma.$disconnect();
  }
...
const restoreSchedulerCursorState = async (
  targetDatabaseUrl: string,
  snapshot: ReadonlyMap<string, { id: string; specId: string; lastTriggeredAt: Date | null; metadata: unknown } | null>,
): Promise<void> => {
  const prisma = await createPrismaClient(targetDatabaseUrl);
...
testWithDatabase("runSchedulerCycle persists scheduler cursors and carries durable ownership metadata", async (targetDatabaseUrl) => {
  const prefix = `rt-scheduler-${Date.now().toString(36)}`;
  const leaseOwner = `${prefix}:scheduler`;
  const priorCursorState = await snapshotSchedulerCursorState(targetDatabaseUrl);
  ...
  try {
    const eligibleFixture = await createFixtureWithOdds(targetDatabaseUrl, prefix, "1", {
      selectionOverride: "force-include",
    });
    const blockedFixture = await createFixtureWithOdds(targetDatabaseUrl, prefix, "2", {
      homePrice: 1.05,
      drawPrice: 7.2,
      awayPrice: 15,
    });
    ...
    try {
      ...
      assert.equal(manifestTasks.filter((task) => task.kind === "research").length, 1);
      assert.equal(
        manifestTasks.filter((task) => task.kind === "fixture_ingestion" || task.kind === "odds_ingestion").length,
        2,
      );
      assert.equal(
        manifestTasks.every(
      ...
      assert.equal(
        secondManifestTasks.filter((task) => task.kind === "fixture_ingestion" || task.kind === "odds_ingestion").length,
        0,
      );
      assert.equal(secondManifestTasks.filter((task) => task.kind === "research").length, 1);
    } finally {
      await prisma.$disconnect();
    }
  } finally {
...
testWithDatabase("runDispatcherCycle scopes work to a single manifest and publisher uses only current manifest prediction task ids", async (targetDatabaseUrl) => {
  const prefix = `rt-dispatcher-${Date.now().toString(36)}`;
  const historicalPrefix = `${prefix}-historical`;
  const now = new Date("2000-01-01T10:01:00.000Z");

  await cleanupRuntimeArtifacts(targetDatabaseUrl, prefix);
  await cleanupRuntimeArtifacts(targetDatabaseUrl, historicalPrefix);

  try {
    const currentFixtureOne = await createFixtureWithOdds(targetDatabaseUrl, prefix, "1", {
      selectionOverride: "force-include",
    });
    const currentFixtureTwo = await createFixtureWithOdds(targetDatabaseUrl, prefix, "2", {
      selectionOverride: "force-include",
    });
    const historicalFixture = await createFixtureWithOdds(targetDatabaseUrl, historicalPrefix, "1");
    await seedHistoricalPublishedPrediction(targetDatabaseUrl, historicalPrefix, historicalFixture.fixtureId);
    ...
    try {
      ...
      assert.equal(
        currentManifestTasks.filter((task) => task.kind === "research" && task.status === "succeeded").length,
        2,
      );
      assert.equal(
        currentManifestTasks.filter((task) => task.kind === "prediction" && task.status === "succeeded").length,
        2,
      );
      assert.equal(
        currentManifestTasks.some((task) => task.kind === "validation" && task.status === "succeeded"),
        true,
      );
      ...
      const successfulPredictionTaskIds = asStringArray(dispatcherMetadata?.successfulPredictionTaskIds);
      const selectedPredictionIds = asStringArray(parlayMetadata?.selectedPredictionIds);
...
testWithDatabase("runRecoveryCycle redrives expired durable leases without relying on updatedAt fallbacks", async (targetDatabaseUrl) => {
  const prefix = `rt-recovery-${Date.now().toString(36)}`;
  const taskId = `${prefix}-task`;
  const taskRunId = `${prefix}-run-1`;
  const now = new Date("2000-01-01T00:20:00.000Z");
  const prisma = await createPrismaClient(targetDatabaseUrl);
  const unitOfWork = createPrismaUnitOfWork(prisma);
  ...
  try {
    await unitOfWork.tasks.save(
      createTask({
        ...
        leaseExpiresAt: "2000-01-01T00:05:00.000Z",
        claimedAt: "2000-01-01T00:00:00.000Z",
        lastHeartbeatAt: "2000-01-01T00:01:00.000Z",
    ...
    assert.equal(recoveredTask?.lastErrorMessage, "Recovered expired lease; scheduling redrive.");
    ...
    assert.equal(taskRuns[1]?.error, "Recovered expired lease; scheduling redrive.");
  } finally {
    await prisma.$disconnect();
    await cleanupRuntimeArtifacts(targetDatabaseUrl, prefix);
  }
