...
interface SchedulerServiceConfig {
  ...
  readonly cycleOptions: SchedulerCycleOptions;
}

const serviceName = "hermes-scheduler";
...
const parseFixtureIds = (args: readonly string[]): readonly string[] | undefined => {
  const repeated = readRepeatedArgValues(args, "--fixture-id");
  const csv = readArgValue(args, "--fixture-ids")
    ?.split(",")
    .map((value) => value.trim())
    .filter((value) => value.length > 0) ?? [];
  const fixtureIds = [...repeated, ...csv].filter((value) => value.length > 0);

  return fixtureIds.length > 0 ? fixtureIds : undefined;
};
...
const parseServiceConfig = (
  ...
  const fixtureIds = parseFixtureIds(args);
...
const shouldRunService = (args: readonly string[], env: NodeJS.ProcessEnv): boolean =>
  ...
  env.GANA_HERMES_SCHEDULER_SERVICE === "1";
  ...
  try {
    return await Promise.race([
      ...
      new Promise<RuntimeCycleResult>((_resolve, reject) => {
        timeout = setTimeout(() => {
          reject(new Error(`scheduler cycle timed out after ${cycleTimeoutMs}ms`));
        }, cycleTimeoutMs);
      }),
    ]);
  } finally {
  ...
  try {
    while (!stopping) {
      ...
      const result = await withCycleTimeout(
        runSchedulerService(databaseUrl, config.cycleOptions),
    ...
    logServiceEvent("hermes.cycle.failed", {
      cycleNumber,
      error: error instanceof Error ? error.message : "Unexpected scheduler service error",
    });
    throw error;
  } finally {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (shouldRunService(args, process.env)) {
    await runSchedulerLoop(databaseUrl, parseServiceConfig(args, process.env));
  } else {
